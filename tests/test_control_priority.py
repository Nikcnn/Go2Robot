...
def test_estop_blocks_all_motion() -> None:
    ...
    control = ControlCore(adapter=adapter, max_vx=0.5, max_vy=0.5, max_vyaw=1.0, watchdog_timeout_ms=200)
    control.start()
    try:
...
def test_watchdog_stops_stale_manual_teleop() -> None:
    adapter = DummyAdapter()
    control = ControlCore(adapter=adapter, max_vx=0.5, max_vy=0.5, max_vyaw=1.0, watchdog_timeout_ms=120)
    control.start()
    try:
        assert control.take_manual()
